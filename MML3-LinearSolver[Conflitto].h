...
namespace MML3{
	namespace Solver{
		...
		bool LLT<T,MP,MS,MO>::factorize(matrix_t& A, bool L )
		{
			...
			if (std::is_same<MS,MAT_SHAPE::RE>::value)
			{
				// if is a rectangular symmetric the input paramater L is ignored
				if (std::is_same<MP,MAT_PROP::SYM>::value)
			...
			else
			{
				...
				if (std::is_same<MO,MAT_ORD::COL>::value)
				{
					uplo_ = std::is_same<MS,MAT_SHAPE::LT>::value?'L':'U';
				...
				else
				{
					...
					if (std::is_same<MS,MAT_SHAPE::LT>::value)
					{
						uplo_ = 'L';
		...
		bool LLT<T, MP, MS, MO>::solve(BMAT& X)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						char tmp_uplo = std::is_same<MAT_SHAPE::LT,MS>::value?'U':'L';
